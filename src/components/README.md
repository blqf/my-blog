...
| 属性名        | 含义               | 类型    | 必填 | 默认值 |
| ------------- | ------------------| ------- | --- | ------ |
| bigTitle      | 列表大标题         | String  | 否   | ""    |
| title         | 列表标题           | String  | 否   | ""    |
| total         | 评论的总数         | Number  | 否   | 0    |
| list         | 评论列表数据         | Number  | 否   | []    |
| isLoading     | 列表是否正在加载中  | Boolean | 否   | false |

## 事件

| 事件名 | 含义               | 事件参数1 | 事件参数2 | 参数类型 |
| ------ | ------------------| -------- | --------- | -------- |
| submit | 用户点击提交时触发  | 数据对象 | 回调函数(数据请求完成后执行) | Object  |
